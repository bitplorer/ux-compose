...
> **Diátaxis:** explanation · **Canonical:** `docs/OWNERSHIP.md` · **Layer:** ux-compose  
...
ux-compose  PRODUCT    create-app · build · serve · deploy · doctor
            ...
            + DEV      origin + ui + channel · HMR · CSS sibling --watch · tunnel
                       prepares UXCOMPOSE_STATE_STORE unless REDIS_URL
                       (Channel prefers Redis when both would be set)
...
doctor fail. `Document.use(+Channel.optional)` is the full shell. There is no
Document-absent product path.
...
Cap door is `Channel.boot` (pin ux-channel @ `d0412c6`; Cut C floor
