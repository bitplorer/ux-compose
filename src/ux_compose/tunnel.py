...
* ``cloudflare`` — cloudflared trycloudflare.com URL from stdout

Tunnel starts **after** origin health is green (avoids classic edge 502).
Does not upload secrets; token via flag or NGROK_AUTHTOKEN / TUNNEL_TOKEN.
"""
...
def parse_provider(value: Optional[str]) -> Provider:
    ...
    if raw not in aliases:
        raise ValueError(f"unknown tunnel provider {value!r}; use none|ngrok|cloudflare")
    ...
    *,
    ...
    while time.monotonic() < deadline:
        for p in paths:
            ...
            host_part = f"[{probe}]" if ":" in probe and not probe.startswith("[") else probe
    ...
    raise TimeoutError(
        ...
        f"(tried {', '.join(paths)}). Tunnel 502 usually means this. last={last_err!r}"
    )
...
def _start_ngrok(port: int, token: Optional[str], *, target_host: str = "127.0.0.1") -> TunnelHandle:
    bin_path = shutil.which("ngrok")
    if not bin_path:
        raise FileNotFoundError("ngrok not on PATH — https://ngrok.com/download")
    env = os.environ.copy()
    if token:
...
def _start_cloudflare(
    ...
    if not bin_path:
        raise FileNotFoundError("cloudflared not on PATH")
    env = os.environ.copy()
    if token:
        env.setdefault("TUNNEL_TOKEN", token)
    probe = local_probe_host(target_host)
    proc = subprocess.Popen(
        [bin_path, "tunnel", "--url", f"http://{probe}:{port}", "--no-autoupdate"],
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            ...
            raise RuntimeError(f"cloudflared exited {proc.returncode}: {rest or ''.join(lines[-20:])}")
        ...
        if m:
            return TunnelHandle(provider="cloudflare", public_url=m.group(0), process=proc)
    proc.terminate()
    try:
        proc.wait(timeout=3)
    except Exception:
        proc.kill()
    raise TimeoutError(f"cloudflared had no trycloudflare URL. tail={''.join(lines[-10:])!r}")
