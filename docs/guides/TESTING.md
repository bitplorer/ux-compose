...
|------|------|-------------|
| Scaffold hello Document-path fragment (`id=hello`, no document frame) | `test_cto_scaffold_hello_fragment.py` | GREEN |
...
| Morph payload for `#X` must not embed outer shell/brand bar | `test_cto_fragment_law.py` | GREEN (scaffold/fragment Hello + nested-shell `cto_red`) |
