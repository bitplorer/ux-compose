...
ux-motion, `cek-host>=0.2.0`, `cek-surface>=0.2.0` (`cek_host.catalog`;
wheels before 0.2.0 do not satisfy that floor). Clone path:
...
- A `fragment.py` (or `helpers/`) home for the HTML walker. `_fragment_for_target` prefers ux-dom `extract_by_id` when importable; homemade walker is escape if absent — leftover, not a second serialize
- Dual product paths
...
is not a frozen wire import). Pin: ux-channel @ `f1a93fc` (cek-host 0.2.0;
