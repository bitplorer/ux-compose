...
class Pagination(Component):
    ...
    class_btn_muted = (
        "inline-flex min-h-11 shrink-0 cursor-pointer items-center justify-center rounded-full "
        "border border-stone-200 bg-white px-4 text-sm font-medium text-stone-400 "
        "opacity-50"
    )
    class_list = "m-0 grid list-none gap-1.5 p-0"
    class_item = "rounded-2xl bg-stone-50 px-4 py-3 text-sm"
    class_bar = "mt-1 flex min-w-0 flex-wrap items-center justify-between gap-2"
    class_dots = "order-last flex min-w-0 basis-full flex-wrap justify-center gap-1"
    class_dot = (
