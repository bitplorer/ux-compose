...
| File | Owns |
|------|------|
| `src/ux_compose/cli/serve_dev.py` | origin, `worker_for`, held sockets, supervisor, SIGUSR1, sibling CSS watch + tunnel |
...
| `src/ux_compose/cli/__init__.py` | argv dispatch. `cli/serve.py` owns `serve dev` / `serve prod` / `serve restart-channel` and the extras check |
