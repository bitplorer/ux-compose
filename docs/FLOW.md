...
| **ux-channel** | Intent, Caps, signed control, ASGI peer | Product code (Isolation) |
...
App.mount(package_dir, asgi_app=api)
  → scan routes/ → Behavior.add → unit_registry
  → surfaces_host.attach_page_router(host=…)
       preferred: pure DirectoryRoutes + RouterHooks(resolve_unit=live)
                + thin adapters.fastapi.mount
       batteries: only if host="batteries" (never default for compose)
```

Authors choose host via:

```python
app.use_host("fastapi")          # or "auto"
bundle = app.mount(..., asgi_app=api)
```

No public HostAdapter class. Strategy is invisible.

## 4. Channel Attachment Flow (Isolation)

```
App.use_channel(asgi_app=api)
  → wire/boot.attach_channel
  → Behavior.attach(asgi) owns Channel.boot(asgi)
  → include_router lands on the real FastAPI
  → bridge_actions registers @action names
...
This map is the residual-free contract.
